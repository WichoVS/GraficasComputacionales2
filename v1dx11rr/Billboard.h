...
public:
	BillboardRR(WCHAR* billb, WCHAR* normal, ID3D11Device* D3DDevice, 
		ID3D11DeviceContext* D3DContext, float escala, float minX, float maxX, float minZ, float maxZ)
	{
		for (int fill = 0; fill < 120; fill++)
		{
			posX[fill] = PosXRndm(minX, maxX);
			posZ[fill] = PosZRndm(minZ, maxZ);
		}
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam, float posy, float posx, float posz,
		D3DXVECTOR4 ambiental, D3DXVECTOR4 difusa, D3DXVECTOR4 especular, float fad, float fa, float fas, D3DXVECTOR3 luzdir)
	{
